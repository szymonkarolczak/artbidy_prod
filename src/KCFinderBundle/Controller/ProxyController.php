...
namespace KCFinderBundle\Controller;
...
class ProxyController extends Controller
{
	public function proxyAction($file)
	{
		...
		if ('.' == $path) {
			$path = $this->getParameter('kc_finder_path');
		} else {
			$path = rtrim($this->getParameter('kc_finder_path'),'/') . '/' . $path;
		}
		...
		$config = $this->getParameter('kc_finder_config');
		if (is_array($config)) {
